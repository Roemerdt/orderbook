...
enum class RejectReason { DuplicatedId, UnknownId };
...
class OrderBook {
  ...
  private:
    ...
    template <class BookSide>
    void erase_resting(BookSide &side_map, Price price,
                       LevelQueue::iterator it) {
        auto level = side_map.find(price);
        assert(level != side_map.end() &&
               "index-book desynced: level missing for order");
